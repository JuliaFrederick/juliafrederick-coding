...
gapminder2 <- gapminder %>% filter(continent=="Africa") %>% select(infant_mortality, life_expectancy, population, country)

ggplot(gapminder2,aes(x=infant_mortality,y=life_expectancy,col=country))+
  geom_point()

ggplot(gapminder2,aes(x=population,y=life_expectancy,col=country))+
  geom_point()
```

```{r}
gapminder %>% filter(continent=="Africa", year==2000) %>% select(infant_mortality, life_expectancy, population, country) %>% 
